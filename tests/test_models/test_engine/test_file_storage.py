...
class TestFileStorage(unittest.TestCase):
    ...
    def tearDownClass(cls):
        ...
        del cls.city
        del cls.amenity
OBOBOB        del cls.review

OBOBOB    def test_pep8_FileStorage(self):
        """Test pep8 styling."""
        style = pep8.StyleGuide(quiet=True)
        p = style.check_files(['models/engine/file_storage.py'])
OBOBOB        self.assertEqual(p.total_errors, 0, "fix pep8")

OBOBOB    def test_docstrings(self):
        """Check for docstrings."""
OBOBOB        self.assertIsNotNone(FileStorage.__doc__)
        self.assertIsNotNone(FileStorage.all.__doc__)
        self.assertIsNotNone(FileStorage.new.__doc__)
OBOBOB        self.assertIsNotNone(FileStorage.reload.__doc__)
...
OBOBOB        self.assertIs(obj, FileStorage._FileStorage__objects)
        self.assertEqual(len(obj), 7)

OBOBOB    def test_all_cls(self):
        """Test all method with specified cls."""
        obj = self.storage.all(BaseModel)
OBOBOB        self.assertEqual(type(obj), dict)
...
OBOBOB            self.assertIn("State." + self.state.id, save_text)
            self.assertIn("Place." + self.place.id, save_text)
            self.assertIn("City." + self.city.id, save_text)
OBOBOB            self.assertIn("Amenity." + self.amenity.id, save_text)
            self.assertIn("Review." + self.review.id, save_text)
OBOBOB
    def test_reload(self):
        """Test reload method."""
OBOBOB        bm = BaseModel()
        with open("file.json", "w", encoding="utf-8") as f:
OBOBOB            key = "{}.{}".format(type(bm).__name__, bm.id)
